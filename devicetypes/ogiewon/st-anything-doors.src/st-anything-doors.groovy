...
metadata {
	...
	tiles {
        ...
        standardTile("allWindows", "device.allWindows", width: 1, height: 1, canChangeIcon: true, canChangeBackground: true) {
			state("open", label:'${name}', icon:"st.contact.contact.open", backgroundColor:"#ffa81e")
			state("closed", label:'${name}', icon:"st.contact.contact.closed", backgroundColor:"#79b821")
 		}
        
        main (["frontDoor"])
        details(["frontDoor","backDoor","allWindows","configure"])
	}
}
